...
class GO:
    def __init__(self, n):
        ...
        # my helper parameters
        self.piece_type = None

    # helper constructor
    def new_Go(self, i, j, piece_type):
        next_go = GO(self.size)
        n_piece_type = 3 - piece_type
        n_previous_board = self.board
        n_board = deepcopy(self.board)
        n_board[i][j] = piece_type
        next_go.set_board(n_piece_type, n_previous_board, n_board)
        next_go.remove_died_pieces(n_piece_type)
        return next_go
    ...
    # get possible placements of current go object
    ...
    def heuristic(self):
        return 100 * (self.get_score(self.piece_type) - self.get_score(3 - self.piece_type))
    ...
    def loc_evaluation(self, i, j):
        return (self.get_liberty_score(i, j) + self.get_ally_score(i,j)) * 100
...
def judge(n_move, verbose=False):
    ...
    if go.game_end(piece_type, action):
        # update helper.txt to 1 when one game is end
        write.writeHelper(1)
